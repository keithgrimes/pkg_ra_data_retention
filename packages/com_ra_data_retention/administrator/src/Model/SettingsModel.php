...
class SettingsModel extends ListModel
{
	...
	*/
	...
	protected function getListQuery()
	{
		...
		if (JDEBUG) { JLog::add("[models][settings] call to getListQuery", JLog::DEBUG, "com_ra_data_retention"); }
	
		$query->select($this->getState('list.select','DISTINCT r.*'))
			->from($db->quoteName('#__ra_retention_settings', 'r'));
		...
		$query->where('(r.testmode = ' . $testmode. ')');
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('r.setting LIKE ' . $search);
			}
		}

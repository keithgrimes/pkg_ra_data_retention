...
if (!empty($saveOrder))
{
	$saveOrderingUrl = 'index.php?option=com_ra_data_retention&task=articleretentions.saveOrderAjax&tmpl=component&' . Session::getFormToken() . '=1';
	HTMLHelper::_('draggablelist.draggable');
}

?>

<form action="<?php echo Route::_('index.php?option=com_ra_data_retention&view=articleretentions'); ?>" method="post"
				...
				<table class="table table-striped" id="retentionList">
					...
					<?php foreach ($this->items as $i => $item) :
						...
						<tr class="row<?php echo $i % 2; ?>" data-draggable-group='1' data-transition>
							...
							<td class="text-center">
								<?php echo HTMLHelper::_('jgrid.published', $item->state, $i, 'articleretentions.', $canChange, 'cb'); ?>
							</td>
							
							<td>
								<?php if (isset($item->checked_out) && $item->checked_out && ($canEdit || $canChange)) : ?>
									<?php echo HTMLHelper::_('jgrid.checkedout', $i, $item->uEditor, $item->checked_out_time, 'articleretentions.', $canCheckin); ?>
								<?php endif; ?>
								<?php if ($canEdit) : ?>
									<a href="<?php echo Route::_('index.php?option=com_ra_data_retention&task=articleretention.edit&id='.(int) $item->id); ?>">

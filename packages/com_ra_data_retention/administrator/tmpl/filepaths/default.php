...
if (!empty($saveOrder))
{
	$saveOrderingUrl = 'index.php?option=com_ra_data_retention&task=filepaths.saveOrderAjax&tmpl=component&' . Session::getFormToken() . '=1';
	HTMLHelper::_('draggablelist.draggable');
}

?>

<form action="<?php echo Route::_('index.php?option=com_ra_data_retention&view=filepaths'); ?>" method="post"
			...
			<?php echo LayoutHelper::render('joomla.searchtools.default', array('view' => $this)); ?>
				...
				<table class="table table-striped" id="retentionList">
					...
					<th  scope="col" class="w-1 text-center">
						...
						<th class='left'>
							<?php echo HTMLHelper::_('searchtools.sort',  'COM_RA_DATA_RETENTION_RETENTIONS_FILEPATH', 'r.filepath', $listDirn, $listOrder); ?>
						</th>
						<th class='left'>
							<?php echo HTMLHelper::_('searchtools.sort',  'COM_RA_DATA_RETENTION_RETENTIONS_EXCLUDE', 'r.exclude', $listDirn, $listOrder); ?>
						</th>
					...
					<?php foreach ($this->items as $i => $item) :
						...
						<tr class="row<?php echo $i % 2; ?>" data-draggable-group='1' data-transition>
							...
							<td class="text-center">
								<?php echo HTMLHelper::_('jgrid.published', $item->state, $i, 'filepaths.', $canChange, 'cb'); ?>
							</td>
							
							<td>
								<?php if (isset($item->checked_out) && $item->checked_out && ($canEdit || $canChange)) : ?>
									<?php echo HTMLHelper::_('jgrid.checkedout', $i, $item->uEditor, $item->checked_out_time, 'filepaths.', $canCheckin); ?>
								<?php endif; ?>
								<?php if ($canEdit) : ?>
									<a href="<?php echo Route::_('index.php?option=com_ra_data_retention&task=filepath.edit&id='.(int) $item->id); ?>">
									<?php echo $this->escape($item->filepath); ?>
									</a>
								<?php else : ?>
									<?php echo $this->escape($item->filepath); ?>
								<?php endif; ?>

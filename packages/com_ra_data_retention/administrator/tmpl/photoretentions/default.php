...
if (!empty($saveOrder))
{
	$saveOrderingUrl = 'index.php?option=com_ra_data_retention&task=photoretentions.saveOrderAjax&tmpl=component&' . Session::getFormToken() . '=1';
	HTMLHelper::_('draggablelist.draggable');
}

?>

<form action="<?php echo Route::_('index.php?option=com_ra_data_retention&view=photoretentions'); ?>" method="post"
				...
				<table class="table table-striped" id="retentionList">
					...
					<?php foreach ($this->items as $i => $item) :
						...
						<tr class="row<?php echo $i % 2; ?>" data-draggable-group='1' data-transition>
							...
							<td class="text-center">
								<?php echo HTMLHelper::_('jgrid.published', $item->state, $i, 'photoretentions.', $canChange, 'cb'); ?>
							</td>
							
							<td>
								<?php if (isset($item->checked_out) && $item->checked_out && ($canEdit || $canChange)) : ?>
									<?php echo HTMLHelper::_('jgrid.checkedout', $i, $item->uEditor, $item->checked_out_time, 'photoretentions.', $canCheckin); ?>
								<?php endif; ?>
								<?php if ($canEdit) : ?>
									<a href="<?php echo Route::_('index.php?option=com_ra_data_retention&task=photoretention.edit&id='.(int) $item->id); ?>">

...
if (!empty($saveOrder))
{
	$saveOrderingUrl = 'index.php?option=com_ra_data_retention&task=settings.saveOrderAjax&tmpl=component&' . Session::getFormToken() . '=1';
	HTMLHelper::_('draggablelist.draggable');
}

?>

<form action="<?php echo Route::_('index.php?option=com_ra_data_retention&view=settings'); ?>" method="post"
			...
			<?php echo LayoutHelper::render('joomla.searchtools.default', array('view' => $this)); ?>
				...
				<table class="table table-striped" id="retentionList">
					...
					<th  scope="col" class="w-1 text-center">
						...
						<th class='left'>
							<?php echo HTMLHelper::_('searchtools.sort',  'COM_RA_DATA_RETENTION_RETENTIONS_SETTING', 'r.setting', $listDirn, $listOrder); ?>
						</th>
						<th class='left'>
							<?php echo HTMLHelper::_('searchtools.sort',  'COM_RA_DATA_RETENTION_RETENTIONS_VALUE', 'r.value', $listDirn, $listOrder); ?>
						</th>
					...
					<?php foreach ($this->items as $i => $item) :
						...
						<tr class="row<?php echo $i % 2; ?>" data-draggable-group='1' data-transition>
							...
							<td class="text-center">
								<?php echo HTMLHelper::_('jgrid.published', $item->state, $i, 'settings.', $canChange, 'cb'); ?>
							</td>
							
							<td>
								<?php if (isset($item->checked_out) && $item->checked_out && ($canEdit || $canChange)) : ?>
									<?php echo HTMLHelper::_('jgrid.checkedout', $i, $item->uEditor, $item->checked_out_time, 'settings.', $canCheckin); ?>
								<?php endif; ?>
								<?php if ($canEdit) : ?>
